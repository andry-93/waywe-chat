    ...
    ***************************************************************************** */
    ...
    var DataViewBase = (function (_super) {
        ...
        DataViewBase.prototype.ShowSubFolder = function (id) {
            ...
            this.State.$batch({ path: path, search: "", searchStats: null });
        };
    ...
    var CardsView = (function (_super) {
        ...
        CardsView.prototype.config = function () {
            ...
            var itemSize = skin.listItemHeight < 29
                ? {
                    height: 151,
                    width: 184,
                }
            ...
            return {
                ...
                type: __assign(__assign({}, itemSize), { type: "tiles", templateStart: webix.template("<div\nclass=\"{common.classname()}\" {common.aria()}\nstyle=\"width: {common.width}px; height: {common.height}px; float: left; overflow: hidden; \">") }),
                on: {
                ...
                onClick: {
                    webix_fmanager_card: function (e, id) { return _this.SelectItem(e, id); },
        ...
        CardsView.prototype.CardTemplate = function (obj) {
            ...
            if (obj.type === "folder") {
                ...
                panel = "<div class=\"webix_fmanager_card_panel file\">\n\t\t<span class=\"webix_fmanager_card_name\">" + fileIcon + "<span class=\"file_name_text\">" + obj.value + "</span></span>" + menuIcon + "\n\t\t</div>";
            }
            return "<div class=\"webix_fmanager_card_outer\"><div class=\"webix_fmanager_card\" webix_l_id=\"" + obj.id + "\">" + (preview + panel) + "</div></div>";
        };
        CardsView.prototype.SelectItem = function (e, id) {
            var _a;
            var table = this.WTable;
            var ctrl = e.ctrlKey || e.metaKey;
            var range = e.shiftKey;
            var select = table.getSelectedId(true);
            if (select.length && range) {
                var start = table.getIndexById(select[select.length - 1]);
                var end = table.getIndexById(id);
                if (start === end)
                    return;
                if (start > end)
                    _a = [end, start], start = _a[0], end = _a[1];
                var files = table.data.order.slice(start, end + 1);
                table.select(files, true);
            }
            else {
                if (table.isSelected(id) && ctrl)
                    table.unselect(id);
                else
                    table.select(id, ctrl);
            }
        };
        return CardsView;
    }(DataViewBase));
    ...
    var EditorView = (function (_super) {
        ...
        EditorView.prototype.init = function () {
            var _this = this;
            this.$$("name").setValue(this.File.id);
    ...
    var FoldersView = (function (_super) {
        ...
        FoldersView.prototype.config = function () {
            ...
            var FSStats = {
                ...
                template: function (obj) {
                    ...
                    var svg = "<svg viewBox='0 0 100 10'>\n\t\t\t\t<rect y='1' rx='2' ry='2' width='" + 100 + "' height='4' style='fill:#DFE2E6;' />\n\t\t\t\t<rect y='1' rx='2' ry='2' width='" + (used ||
                        0) + "' height='4' style='fill:" + activeSkinAccentColor + ";' />\n\t\t\t\tSorry, your browser does not support inline SVG.\n\t\t\t\t</svg>";
        ...
        FoldersView.prototype.GetFsStats = function () {
            var _this = this;
            this.app
                .getService("backend")
                .getInfo()
    ...
    var GridView = (function (_super) {
        ...
        GridView.prototype.init = function () {
            ...
            this.on(this.State.$changes, "file", function (v) {
                if (v) {
                    if (_this.WTable.exists(v))
                        _this.WTable.select(v, true);
                }
            });
    ...
    var DoublePanelView = (function (_super) {
        ...
        DoublePanelView.prototype._TrackChanges = function (state, next) {
            var _this = this;
            this.on(state.$changes, "path", function (v) { return (_this.State.path = v); });
    ...
    var TopView = (function (_super) {
        ...
        TopView.prototype.init = function () {
            ...
            if (root.sizeTrigger)
                root.sizeTrigger(640, function (mode) { return _this.SetCompactMode(mode); }, !!this.Compact);
    ...
    var Backend = (function () {
        ...
        Backend.prototype.search = function (id, search) {
            id = id || "/";
            return this._files(id, this.url("files?id=" +
        ...
        Backend.prototype.files = function (id) {
            id = id || "/";
            return this._files(id, this.url("files?id=" + encodeURIComponent(id)));
        };
        Backend.prototype._files = function (id, urlStr) {
            var data = webix.ajax(urlStr).then(function (res) { return res.json(); });
            return this.getInfo().then(function () { return data; });
        };
        Backend.prototype.folders = function () {
            var data = webix.ajax(this.url("folders"));
        ...
        Backend.prototype.readText = function (id) {
            return webix
                .ajax(this.url("text?id=" + encodeURIComponent(id)))
        ...
        LocalData.prototype.updateFile = function (oldId, data, newId) {
            ...
            if (hierarchy.exists(oldId)) {
                ...
                if (newId && oldId != newId) {
                    ...
                    hierarchy.data.each(function (l) {
                        if (l.id.indexOf(oldId) !== -1) {
                            hierarchy.data.changeId(l.id, l.id.replace(oldId, newId));
                        }
                    });
                }
            }
                ...
                return _this.app
                    ...
                    return _this.backend()
                        ...
                        if (!res.invalid) {
                            ...
                            var sel = state.selectedItem.filter(function (s) { return s.id !== f; });
                            state.selectedItem = sel;
                        }
                    });
                });
            });
            ...
            prompt({
                ...
                if (name && name !== file.value)
                    _this.backend()
                        ...
                        if (_this.state.path === oldId) {
                            _this.state.path = res.id;
                            _this.app.callEvent("pathChanged", [oldId, res.id]);
                        }
                        if (state.selectedItem[0].id === oldId) {
                            state.selectedItem[0] = __assign(__assign({}, state.selectedItem[0]), { value: name, id: res.id });
                        }
                    });
            });
        };
